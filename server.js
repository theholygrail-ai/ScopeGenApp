...
async function getPricingSheetData(retries = 3) {
    ...
    let authOptions;
    if (rawCredentials) {
        let cleanedCreds = rawCredentials
            .trim()
            .replace(new RegExp('^\\uFEFF'), ''); // remove BOM if present

        // Remove wrapping quotes if the value was provided as a quoted string
        if ((cleanedCreds.startsWith('"') && cleanedCreds.endsWith('"')) ||
            (cleanedCreds.startsWith("'") && cleanedCreds.endsWith("'"))) {
            cleanedCreds = cleanedCreds.slice(1, -1);
        }

        // Convert escaped newline characters to real newlines
        cleanedCreds = cleanedCreds.replace(/\\n/g, '\n').replace(/\\r/g, '\r');
...
app.get('/export/pptx/run/:runId', async (req, res) => {
    if (!pool) return res.status(400).json({ message: 'DATABASE_URL not configured' });
    try {
        const { runId } = req.params;
        const slidesRes = await pool.query('SELECT * FROM slides WHERE run_id=$1 ORDER BY created_at', [runId]);
        if (slidesRes.rowCount === 0) return res.status(404).json({ message: 'Run not found' });
        const pptx = new PPTX();
        pptx.layout = 'LAYOUT_WIDE';
